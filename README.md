...
1. Клоинруйте этот репозиторий:

     ```bash
     git clone https://github.com/nikuIin/go_bath_auth
     ```
2. Убедитесь, что у вас есть файл `.env` в корневой директории. Для примера создания `.env` файла нужно взять `.env_example` из исходного кода проекта:
3. Запустите билд `docker` контейнеров:
